...
// Take return a new slice that contains the given number of elements from the left side.
// When the given number is zero or negative, it returns nil.
// When the given slice is empty, it returns nil.
// When the given number is greater than the length of the slice, it returns the
// original slice.
func Take[S ~[]E, E any](s S, n int) S {
	if n < 0 {
		return nil
	}
